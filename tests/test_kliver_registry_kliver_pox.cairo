...
fn deploy_kliver_pox(registry: ContractAddress) -> IKliverPoxDispatcher {
    let cls = declare("KliverPox").unwrap().contract_class();
    let mut calldata = ArrayTrait::new();
...
#[test]
fn test_register_session_mints_in_kliver_pox_and_verifications() {
    // Deploy dependencies
    ...
    // Deploy KliverPox with registry as authorized minter
    let klive = deploy_kliver_pox(registry_addr);

    // Set KliverPox address (only owner)
    let owner_iface = IOwnerRegistryDispatcher { contract_address: registry_addr };
    start_cheat_caller_address(registry_addr, OWNER());
    owner_iface.set_kliver_pox_address(klive.contract_address);
    ...
    // Register a session via registry (only owner), this mints in KliverPox
    ...
    // Verify KliverPox has the metadata
