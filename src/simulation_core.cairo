...
pub trait ISimulationCore<TContractState> {
    fn register_simulation(ref self: TContractState, simulation_id: felt252, daily_amount: u256, release_hour: u8);
    ...
    fn get_claimable_tokens(self: @TContractState, simulation_id: felt252, user: ContractAddress) -> u256;
...
mod SimulationCore {
    ...
    impl SimulationCoreImpl of super::ISimulationCore<ContractState> {
        fn register_simulation(ref self: ContractState, simulation_id: felt252, daily_amount: u256, release_hour: u8) {
            self.only_owner();
            ...
            self.simulations.entry(simulation_id).write(
                super::Simulation { token_id, daily_amount, active: true, release_hour }
            );
        ...
        fn claim_tokens(ref self: ContractState, simulation_id: felt252) {
            ...
            assert(self.can_claim_now(simulation_id), 'Claim time not reached');
        ...
        fn get_time_until_next_claim(
            ...
            let can_claim_now = self.can_claim_now(simulation_id);
